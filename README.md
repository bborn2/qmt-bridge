> 本仓库为 fork 版本，仅在 Windows 上使用，不考虑跨平台。以下为我在原项目基础上的修改说明与使用要点。
...
- **xtquant 不会自动下载**：本项目依赖 xtquant，但它 **不会** 随 `uv sync` 自动安装。请自行到迅投官方页面下载最新包并安装：
  <https://dict.thinktrader.net/nativeApi/download_xtquant.html?id=e2M5nZ>
- **QMT 安装目录不要放在需要权限的路径**：安装 QMT 客户端时，**不要**装到 `Program Files`、`Program Files (x86)` 等需要管理员权限的目录，否则 xtquant 读写数据会失败。
### 快速上手（uv + just）
...
> Windows 服务端需要 `xtquant`，但它 **不会** 随上面的命令自动安装。请按「重要提示」到迅投官方页面下载最新包并手动安装。
